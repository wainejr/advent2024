...
fn walk(idx: usize, path: []usize, hike_final: []bool) !usize {
    ...
    for (possible_dirs) |dir| {
        ...
        n_hikes += try walk(pos_idx, new_path, hike_final);
    }
    return n_hikes;
}
...
fn check_node_hike(idx: usize) !usize {
    ...
    const n_hikes = try walk(idx, @constCast(@ptrCast(&ini_path)), @ptrCast(&hike_final));
