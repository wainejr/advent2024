...
fn check_antinodes(char: u8) void {
    ...
    for (idxs_char, 0..) |i, idx| {
        ...
        for (idxs_char[idx + 1 ..]) |j| {
            ...
            for (0..@intCast(@max(N_COLS, N_ROWS))) |cmul| {
                const mul: i32 = @intCast(cmul);
                const anti: [2]i32 = .{ posj[0] - dist[0] * mul, posj[1] - dist[1] * mul };
                if (is_valid_pos(anti[0], anti[1])) {
                    const idx_anti = pos2idx(anti[0], anti[1]);
                    // std.debug.print(
                    //     "ANTI {any} {any} {any}\n",
                    //     .{ idx_anti, idx2pos(idx_anti), anti },
                    // );
                    AntiNode[idx_anti] = true;
                } else {
                    break;
                }
            }
            for (1..@intCast(@max(N_COLS, N_ROWS))) |cmul| {
                const mul: i32 = @intCast(cmul);
                const anti: [2]i32 = .{ posj[0] + dist[0] * mul, posj[1] + dist[1] * mul };
                if (is_valid_pos(anti[0], anti[1])) {
                    const idx_anti = pos2idx(anti[0], anti[1]);
